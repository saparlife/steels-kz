    ...
    <div>
      <div className="flex items-center justify-between mb-6">
        <div className="flex items-center gap-3">
          ...
          <h1 className="text-2xl font-bold text-gray-900">Категории FAQ</h1>
          <span className="px-2 py-1 text-sm bg-orange-100 text-orange-700 rounded-full">{items.length}</span>
        </div>
        <Link href="/admin/faq-categories/new" className="flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600">
      ...
      <div className="bg-white rounded-lg shadow">
        {loading ? (
          ...
          <div className="p-8 text-center">
            ...
            <h3 className="text-lg font-medium text-gray-900 mb-2">Нет категорий FAQ</h3>
            <p className="text-gray-500 mb-6">Создайте категории для организации вопросов</p>
            <Link href="/admin/faq-categories/new" className="inline-flex items-center gap-2 px-4 py-2 bg-orange-500 text-white rounded-lg hover:bg-orange-600">
              <Plus className="w-4 h-4" />
              Добавить категорию
            </Link>
          </div>
            ...
            <tbody>
              {items.map((item) => (
                <tr key={item.id} className="border-b border-gray-100 hover:bg-gray-50">
                  <td className="px-4 py-3 font-medium text-gray-900">{item.name_ru}</td>
                  ...
                  <td className="px-4 py-3 text-right">
                    <Link href={`/admin/faq-categories/${item.id}`} className="p-2 text-gray-500 hover:text-orange-500">
